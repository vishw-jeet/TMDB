...
<div class="card">
  <figure class="movieCard m-0">
    <img src="https://image.tmdb.org/t/p/w1280//${movieArray[i].poster_path}" alt="cardImg"title="cardImg">
    <figcaption>
      <div class="movieTitle">
        <div class="row">
          ...
          <div class="col-2"  id="rating">
            <span class="rating"> ${movieArray[i].vote_average}</span> 
          </div>
        </div>
      </div>
    </figcaption>
...
var colorChange=document.querySelectorAll(".rating");
cl(colorChange.length);
for(let i=0; i<=colorChange.length - 1; i++){
  cl(movieArray)
  if(movieArray[i].vote_average >= 7){
    window.addEventListener("load",()=>{
      colorChange[i].setAttribute("class","color-change")
    });
  }else if(movieArray[i].vote_average < 7 && movieArray[i].vote_average >= 5){
    window.addEventListener("load",()=>{
      colorChange[i].setAttribute("class","yellow")
    });
  }else{
    window.addEventListener("load",()=>{
      colorChange[i].setAttribute("class","color-red")
    })
